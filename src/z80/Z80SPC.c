...
#ifdef COLEM
extern byte *RAM;
INLINE byte RdZ80(uint16 A) { return(RAM[A]); }
#endif
#ifdef MG
extern byte *Page[];
INLINE byte RdZ80(uint16 A) { return(Page[A>>13][A&0x1FFF]); }
#endif
#ifdef FMSX
extern byte *RAM[],PSL[],SSLReg;
INLINE byte RdZ80(uint16 A)
{
...
/*************************************************************/
void IntZ80(Z80 *R,uint16 Vector)
{
  if((R->IFF&IFF_1)||(Vector==INT_NMI))
  {
    ...
    if(R->IFF&IFF_IM2)
    {
      ...
      Vector=(Vector&0xFF)|((uint16)(R->I)<<8);
      /* Read the vector */
